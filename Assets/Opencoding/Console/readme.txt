...
== Version 3.0.0 ==
IMPORTANT! To upgrade to this version, please delete the Assets/Opencoding/Console/DLLs folder first. The code for the DLLs
is now included, in the Assemblies folder.

Changes
- The source code for the console is now included
- Support has been added for the new Input System
- Performance improvements have been made to the process of registering new commands

Bug fixes
- Exporting logs to HTML or emailing them will now handle <, > and & symbols correctly
- If the Mail app has been removed on iOS or has no accounts, the Email... button will no longer be shown. Sharing can be used instead (see above).
- Fixed a race condition that could cause errors to occur occasionally if log messages were being output on a non-main thread
- Worked around a Unity bug causing the keyboard not to open on Android devices with Unity 2022
- Fixed a crash on Android devices caused by a Unity bug (thanks to Kazoo Games for the fix)
