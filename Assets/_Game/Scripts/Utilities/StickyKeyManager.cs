...
public class StickyKeyManager : MonoBehaviour
{
    private static StickyKeyManager _instance;

    public static StickyKeyManager Instance { get { return _instance; } }
    ...
    {
        initialized = true;
        StickyKeyManager.SetStickyKeys(false);
    }

    void OnDestroy()
    {
        if (initialized)
            StickyKeyManager.SetStickyKeys(true);
    }

    public static void SetStickyKeys(bool enable)
    {
#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
        StickyKeyManager.AllowAccessibilityShortcutKeys(enable);
#endif
    }


#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN    /// <summary>
    ...
    public struct FILTERKEY
    {
        public uint _cbSize;
        public uint _dwFlags;
        public uint _iWaitMSec;
        public uint _iDelayMSec;
        public uint _iRepeatMSec;
        public uint _iBounceMSec;
    }
    ...
    public static void AllowAccessibilityShortcutKeys(bool bAllowKeys)
    {
        ...
        {
            StartupStickyKeys._cbSize = SKEYSize;
            StartupToggleKeys._cbSize = SKEYSize;
            StartupFilterKeys._cbSize = FKEYSize;
        ...
        else
        {
            ...
            if ((skOff._dwFlags & SKF_STICKYKEYSON) == 0)
            {
                // Disable the hotkey and the confirmation 
                skOff._dwFlags &= ~SKF_HOTKEYACTIVE;
                skOff._dwFlags &= ~SKF_CONFIRMHOTKEY;
                SystemParametersInfo(SPI_SETSTICKYKEYS, SKEYSize, ref skOff, 0);
            }
            SKEY tkOff = StartupToggleKeys;
            if ((tkOff._dwFlags & TKF_TOGGLEKEYSON) == 0)
            {
                // Disable the hotkey and the confirmation 
                tkOff._dwFlags &= ~TKF_HOTKEYACTIVE;
                tkOff._dwFlags &= ~TKF_CONFIRMHOTKEY;
                SystemParametersInfo(SPI_SETTOGGLEKEYS, SKEYSize, ref tkOff, 0);
            }

            FILTERKEY fkOff = StartupFilterKeys;
            if ((fkOff._dwFlags & FKF_FILTERKEYSON) == 0)
            {
                // Disable the hotkey and the confirmation 
                fkOff._dwFlags &= ~FKF_HOTKEYACTIVE;
                fkOff._dwFlags &= ~FKF_CONFIRMHOTKEY;
